...
--[[ @POSEIDON_LUA: BEGIN ]]
  ...
  Struct_IdDecList = (lpeg.V("Struct_IdDec")^1 + lpeg.Cc(nil)) / tltype.Table;
...
--[[ @POSEIDON_LUA: BEGIN ]]
  ...
  MallocExp = lpeg.Cp() * tllexer.kw("malloc") * tllexer.symb("(") * 
		(	
			(lpeg.V("PtrType") * 
				tllexer.symb(",") *
				lpeg.V("Expr")
...
function traverse_Modified_Call ( env, exp )
...
end --end traverse_Modified_Call
...
--[[ @POSEIDON_LUA: BEGIN ]]
	...
	local found, status, msg = traverse_Modified_Call( env, exp )
